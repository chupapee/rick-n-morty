...
import { Select, Wrap, Options } from "./style"
...
export const Filter = () => {
  ...
  const filterList = ['status, location, name, gender, species']
  return (
    <>
      <Wrap>
      <Select>
        ...
        {opened
          ? (
              <>
                <img
                  src="https://i.postimg.cc/5tjmCHPF/pngegg.png"
                  alt="select"
                />
                <Options>
                  {episodes.map(({episode, id}) => (
                    <span onClick={() => chooseEpisode(id)} key={id}>{episode}</span>
                  ))}
                </Options>
              </>
            )
